...
export function validateSchema(data: unknown): SchemaValidationResult {
  ...
  // ts
  if (typeof obj.ts !== "number" || !Number.isInteger(obj.ts) || (obj.ts as number) < 0) {
  ...
  if (
    typeof obj.p !== "string" ||
    !(VALID_PERFORMATIVES as readonly string[]).includes(obj.p)
  ) {
    errors.push(
      `p must be one of: ${[...VALID_PERFORMATIVES].join(", ")}. Got ${JSON.stringify(obj.p)}`,
    );
  }
  ...
  if (typeof obj.to === "string") {
    ...
    if (obj.to.length === 0) {
      ...
      for (let i = 0; i < obj.to.length; i++) {
        if (typeof obj.to[i] !== "string" || (obj.to[i] as string).length === 0) {
  ...
  if (obj.p === "DLGT" && typeof obj.body === "object" && obj.body !== null) {
    ...
    if (typeof body.d === "object" && body.d !== null) {
      ...
      if (
        typeof d.delegation_mode !== "string" ||
        !(VALID_DELEGATION_MODES as readonly string[]).includes(d.delegation_mode)
      ) {
        errors.push(
          `DLGT messages require body.d.delegation_mode to be one of: ${[...VALID_DELEGATION_MODES].join(", ")}`,
        );
      }
    }
  }
